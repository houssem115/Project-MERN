...
componentDidMount(){
    Axios.get('http://localhost:4000/findMaisonId/'+this.props.match.params.id).then(res=>this.setState({
        name:res.data.name,
   ...
   e.preventDefault()
        ...
        Axios.put("http://localhost:4000/EditMaison/"+id,{name:this.state.name,categori:this.state.categori,prix:this.state.prix,quantite:this.state.quantite}).then(res=>
this.props.history.push('/admin/maison')
 ) }  
    render() {
        return (
    <MDBContainer>
    <MDBRow>
      <MDBCol md="6">
        <form>
          <p className="h4 text-center mb-4">Edit Decor Maison</p>
          ...
          <br />
                    <MDBBtn onClick={(e)=>this.EditMaison(this.props.match.params.id,e)} color="warning" outline type="submit">
                      Edit
