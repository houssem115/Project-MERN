...
componentDidMount(){
    Axios.get('http://localhost:4000/findjardinId/'+this.props.match.params.id).then(res=>this.setState({
        name:res.data.name,
   ...
   e.preventDefault()
        ...
        console.log('edit')
        Axios.put("http://localhost:4000/EditJardin/"+id,{name:this.state.name,categori:this.state.categori,prix:this.state.prix,quantite:this.state.quantite}).then(res=>
this.props.history.push('/admin')
    ...
    <MDBRow>
      <MDBCol md="6">
        <form>
          <p className="h4 text-center mb-4">Edit Decor Jardin</p>
          ...
          <br />
                    <MDBBtn onClick={(e)=>this.Editjardin(this.props.match.params.id,e)} color="warning" outline type="submit">
                      Edit
